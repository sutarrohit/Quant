...
| 1-02-01 | 02 | 2 | VALID-01, DATA-03 | T-01-07 | DB-side UUID defaults; explicit `@map` on every new field; no dedup mechanism at either snapshot level | structural | `pnpm --filter @repo/prisma exec prisma validate` | ❌ W0 | ⬜ pending |
| 1-02-02 | 02 | 2 | VALID-01, DATA-03 | T-01-05 | Migration applied to a live DB; physical column names confirmed snake_case from `information_schema` | integration | `pnpm --filter @repo/prisma exec prisma migrate deploy` | ❌ W0 | ⬜ pending |
